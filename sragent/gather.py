...
def reannotate(project_id,
    ...
    if Path(f'sragent_output/annotation_FULL.csv').exists():
        ...
        annotation = pd.read_csv('sragent_output/annotation_FULL.csv', keep_default_na=False)
    else:
        raise ValueError("No annotation file found, please run gather with annotate = True first")
    ...
    if Path(f'sragent_output/{project_id}_summary.txt').exists():
        ...
        raise ValueError(f"No summary file found for {project_id}, please run gather with annotate = True first")

    if Path(f'sragent_output/metadata.csv').exists():
        meta = pd.read_csv('sragent_output/metadata.csv', keep_default_na=False)
    else:
    ...
    outdf = pd.concat([annotation[annotation['project_id'] != project_id], outdf])
